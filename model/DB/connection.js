...
const CoordinatorsSchema = new Schema(CoordinatorObj, { collection: "Coordinators", timestamps: true });

connection.getCollection = collectionName => {
    const DB_HOST = "mongodb+srv://itcell:dpo65720@cluster0.vnukk.mongodb.net";
    return mongoose.connect(`${DB_HOST}/SRGP`,
        { useNewUrlParser: true, useUnifiedTopology: true }).then((db) => {
            switch (collectionName) {
                ...
                case COLLECTION_NAME.COORDINATORS: return db.model(collectionName, CoordinatorsSchema);
            }
        }).catch(err => {
