...
const { driveUpload, driveDownload } = require('../middleware/gdrive.middleware')
...
userRouter.get("/coordinators", (req, res, next) => {
...
userRouter.put("/file-upload/:refKey",
    ...
    upload.single('file'),
    (req, res, next) => {
        driveUpload(req.file.filename, req.file.destination).then(fileId => {
...
userRouter.post("/download", userAuth, (req, res, next) => {
    driveDownload(req.body.fileId, req.body.fileName).then(filePath => {
