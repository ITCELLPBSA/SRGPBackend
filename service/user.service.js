        ...
        .then(response => {
            ...
            return userModel.getAllGrievances()
                .then(response => {
                    ...
                    if (userData.userRole == "1") { snapshot.dailyGrievances = response.filter(grievance => grievance.status == '01' || grievance.status == '07' || grievance.status == '-02').length; }
                    if (userData.userRole == "2") { snapshot.dailyGrievances = response.filter(grievance => (grievance.status == '02' || grievance.status == '06' || grievance.status == '-01') && grievance.departments.APODept == userData.userDepartment).length; }
                    if (userData.userRole == "3") { snapshot.dailyGrievances = response.filter(grievance => grievance.status == '00' || grievance.status == '08' || grievance.status == '-03').length; }
                    if (userData.userRole == "4") { snapshot.dailyGrievances = response.filter(grievance => (grievance.status == '03' || grievance.status == '05') && grievance.departments.CHOSDept == userData.userDepartment).length; }
                    if (userData.userRole == "5") { snapshot.dailyGrievances = response.filter(grievance => (grievance.status == '04') && grievance.departments.APODept == userData.userDepartment).length; }
                    if (snapshot) return snapshot;
                    throw new ApiError("User not found", 404);
                });
        });
...
userService.getCoordinators = () => {
    return coordinatorModel.getCoordinators()
        .then(response => {
            if (response) return response;
            throw new ApiError("Cannot Get Coordinator List", 500);
        });
}
