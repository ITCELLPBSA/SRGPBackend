...
adminService.bulkInsert = (data, userRole) => {
    if (userRole > 5) { throw new ApiError("Unauthorized", 401); }
    return employeeModel.bulkInsert(data)
        .then(response => ({ message: `${response} Employee added successfully.` }))
}
adminService.bulkDelete = (data, userRole) => {
    if (userRole > 5) { throw new ApiError("Unauthorized", 401); }
    return userModel.bulkDelete(data).then(result => {
        if (result) {
            return { messageUser: "Users deleted successfully" }
        }
        throw new ApiError("Cannot delete users from login table", 500);
    }), employeeModel.bulkDelete(data)
        .then(res => {
            return res
        })
        .then(response => ({ message: `${data.toDelete.length} Employee Deleted successfully.` }))
}
